...
parser = argparse.ArgumentParser(description= "Plots a burndownchart and optionally the time that actually went into the tasks of the sprint")
parser.add_argument("data", help="Data file with issue number, planned time in minutes, day of completion, and - optionally - actually used time in minutes")
parser.add_argument("graphs", type=int, nargs='?', default=2, help="Number of graphs to plot: either 2 (ideal and actual) or 3 (plus actual time)")
parser.add_argument("firstdayofsprint", nargs='?', default="mo", help="Either mo, di, mi, do, fr, sa, or so")
parser.add_argument("duration", type=int, nargs='?', default=7, help="Duration of sprint in days")
...
print("number of issues = \t", len(issues), "\nnumber of unique issues = ", len(unique_issues))
...
burnt_days_arr = np.array(minutes2days(burntdown))
...
if args.graphs == 3:
    ...
    if burnt_actualminutes != [0]*(duration+1):
        burnt_days_actual_arr = np.array(minutes2days(burntdown_actual))
